...
import 'package:advance_image_picker/widgets/common/button.dart';
import 'package:advance_image_picker/widgets/common/grid_line_painter.dart';
...
const int kBottomControlPanelHeight = 220;
...
class _ImagePickerState extends State<ImagePicker>
  ...
  List<Widget>? _guidelineImages = [];
  int _imageGuidelineIndex = 0;
  ...
  void initState() {
    ...
    _guidelineImages = _configs.guideLineImages;
    ...
    switch (_configs.doneButtonStyle) {
      ...
      case DoneButtonStyle.iconButton:
        return IconButton(
          ...
          onPressed: (_selectedImages.isNotEmpty) ? _doneButtonPressed : () {},
        );
    }
  }
                    ...
                    child: GestureDetector(
                      ...
                      child: Stack(
                        children: [
                          ...
                          if (_isGridLineEnabled && _isFullscreenImage)
                            Positioned.fill(
                              child: _buildGridLines(),
                            ),
                          if (_isImageGuidelineOn)
                            Positioned.fill(
                                top: -100,
                                child: _guidelineImages![_imageGuidelineIndex]),
                        ],
                      ),
                    ))
                : Container();
  ...
  Widget _buildGridLines() {
    return CustomPaint(
      ...
      painter: GridLinesPainter(divisions: 3), // 3x3 grid lines
    );
  }

  void toggleGuidelines() {
    setState(() {
      _isImageGuidelineOn = !_isImageGuidelineOn;
    });
  }
                        ...
                        ? () async {
                            ...
                            if (!(_controller?.value.isTakingPicture ?? true)) {
                              try {
                                ...
                                _imageGuidelineIndex =
                                    (_imageGuidelineIndex + 1) %
                                        _guidelineImages!.length;
                                setState(() {});
                              } on CameraException catch (e) {
          ...
          child: Column(
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text('Enable gridlines',
                      style: TextStyle(color: Colors.white)),
                  const SizedBox(width: 50),
                  Switch.adaptive(
                      value: _isGridLineEnabled,
                      onChanged: (value) {
                        setState(() {
                          _isGridLineEnabled = value;
                        });
                      })
                ],
              ),
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text('Enable guidelines',
                      style: TextStyle(color: Colors.white)),
                  const SizedBox(width: 50),
                  Switch.adaptive(
                      value: _isImageGuidelineOn,
                      onChanged: (value) {
                        setState(() {
                          _isImageGuidelineOn = value;
                        });
                      })
                ],
              ),
